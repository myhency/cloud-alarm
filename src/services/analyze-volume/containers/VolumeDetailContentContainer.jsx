...
import { loadVolumeDataList } from "../../../state/analyzeVolumeSlice";
        ...
        <TableBody>
          {result.map((volumeData) => {
            const volumeBy =
              Math.round(
                (volumeData.volume /
                  (volumeData.numberOfOutstandingShares * 1000)) *
                  100 *
                  100
              ) / 100;
            const amount = Math.round(
              (volumeData.volume * volumeData.closingPrice) / 100000000
            );
            ...
            if (volumeData.marketType === marketType) {
              return (
                <TableRow id={volumeData.id} key={volumeData.id}>
                  ...
                  <TableCell scope="row" padding="default" align="right">
                    <Box display="flex" flexDirection="column">
                      <Typography>
                        {new Intl.NumberFormat("ko-KR").format(
                          volumeData.closingPrice
                        )}
                      </Typography>
                    </Box>
                  </TableCell>
                  <TableCell scope="row" padding="default" align="right">
                    <Box display="flex" flexDirection="column">
                      <Typography
                        style={{
                          color:
                            volumeData.fluctuationRate > 0 ? "red" : "blue",
                        }}
                      >
                  ...
                  <TableCell scope="row" padding="default" align="right">
                    <Box display="flex" flexDirection="column">
                      <Typography>
                        {new Intl.NumberFormat("ko-KR").format(
                          Math.round(volumeData.marketCap)
                        )}
                      </Typography>
                    </Box>
                  </TableCell>
